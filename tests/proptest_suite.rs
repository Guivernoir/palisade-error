...
proptest! {
    ...
    fn sanitized_macro_properties(s in "\\PC*") {
        ...
        // If original was short, should be preserved
        if s.len() <= 256 {
...
proptest! {
    ...
    fn ring_buffer_memory_bounded(
        ...
        assert!(logger.memory_usage_bytes() <= max_memory);
    }
    ...
    fn ring_buffer_queries_stable(
        ...
        let _ = logger.len();
        let _ = logger.memory_usage_bytes();
    }
}
...
// ============================================================================
// OBFUSCATION PROPERTIES (if feature enabled)
// ============================================================================
